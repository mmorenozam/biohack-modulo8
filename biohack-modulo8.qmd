...
![](images/florian-klauer-mk7D-4UCfmg-unsplash.jpg){.image-left}
...
-   En 2022, el grupo de desarrolladores de Posit (RStudio), lanzó Quarto como la siguiente generación de `rmarkdown`. Entre sus ventajas cuenta:
...
::: incremental
-   Para instalar la última versión disponible de Quarto ingresa a [este enlace](https://quarto.org/docs/download/){target="_blank"} y descarga el instalador de acuerdo a tu sistema operativo.

-   Sigue las intrucciones sin cambiar los defaults e instala Quarto.

-   Para el correcto funcionamiento de Quarto, es necesario que cuentes con los paquete `{knitr}` y `{rmarkdown}`.
...
## Opciones en los fragmentos de Código {.smaller}
...
-   `error: true` permite el terminar el renderizado del documento así se encuentren errores en el código. 
...
![Logo Biohack](images/icon1.png){fig-align="center" width=15%}
```
...
-   Funciona sin mayores problemas para la mayoría de plots producidos tanto por gráficos de R base, `{ggplot2}` y otros paquetes. Aunque, con ciertas excepciones como aquellos plots producidos en `{plotly}` (que a veces requieren de una versión específica de `{knitr}`).
...
## Controlando el tamaño de una figura {.smaller}
...
1.  Fijar `fig-width: 6` (6 pulgadas) y `fig-asp: 0.618` (la proporción áurea) para todas las figuras (en el encabezado).
...
Luego, si deseáramos cambiar una figura individual, lo hacemos ajustando `fig-asp` por fragmentos individuales.
```{r}
#| echo: fenced
#| fig-asp: 0.3
...
Recomendaciones tomadas del libro ["R for Data Science"](https://r4ds.hadley.nz/){target="blank_"}
...
Foto portada por <a href="https://unsplash.com/@florianklauer?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash">Florian Klauer</a> en <a href="https://unsplash.com/photos/black-fayorit-typewriter-with-printer-paper-mk7D-4UCfmg?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash">Unsplash</a>
  
  
Foto final por <a href="https://unsplash.com/@kipfarl?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash">Julian Rösner</a> en <a href="https://unsplash.com/photos/a-long-dock-with-benches-in-the-water-2knUR_Kyq24?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash">Unsplash</a>
...
![](images/julian-rosner-2knUR_Kyq24-unsplash.jpg){.image-right}
